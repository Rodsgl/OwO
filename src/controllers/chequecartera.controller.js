import { ChequesCartera } from "../models/ChequesCartera.js";

export const getCheques = async (req, res) => {
  try {
    const cheque = await ChequeCartera.findAll();
    res.json(cheque);
  } catch (error) {
...
export const getCheque = async (req, res) => {
  try {
    const { id } = req.params;
    const cheque = await ChequeCartera.findOne({
      where: {
        id: id,
      },
    });
...
export const createCheque = async (req, res) => {
  ...
  try {
    const newCheque = await CategoriaFlujoCaja.create({
      id: id,
...
export const updateCheque = async (req, res) => {
  try {
    ...
    const mod = await ChequeCartera.findByPk(id);
